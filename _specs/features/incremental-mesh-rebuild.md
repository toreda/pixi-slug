...
4. `indexBuffer.setDataWithSize(indices.subarray(0, liveIndexCount), liveIndexCount, true)` — same upload path, but the buffer's `data` must be a **live-length view**, not the capacity array. PIXI's `gl.drawElements` count is `indexBuffer.data.length` (A4), so passing the full array draws the stale tail after a shrink (A5). Implemented in `SlugText._uploadLiveIndices`, which reuses the previous view when the live count is unchanged.
...
| A4 | `gl.drawElements` count defaults to `geometry.indexBuffer.data.length` — the **JS typed-array length**, not the `setDataWithSize` size | ✅ Verified (corrected 2026-09-11) | 2026-09-11 | pixi.js 8.17.1 | `GlGeometrySystem.js:265, 267` — `size \|\| geometry.indexBuffer.data.length`; `GlMeshAdaptor.js:50` calls `encoder.draw` with no `size`; `Geometry` has no `indexCount` field | **The original reading ("live, not cached") was wrong.** `_updateSize` bounds only the `bufferSubData` upload. If `indexBuffer.data` is the capacity-sized array, every frame draws capacity-many indices. Fix: `indexBuffer.data` must be a `subarray` view of the capacity array whose length is the live index count (`SlugText._uploadLiveIndices`). |
| A5 | Updating the index buffer with a smaller live `data.length` shrinks the draw count on the next frame | ✅ Verified (corrected 2026-09-11) | 2026-09-11 | pixi.js 8.17.1 | A4 + `Buffer.js:127` shrink branch: a shorter view under `shrinkToFit: false` emits `"update"` only, and `GlBufferSystem.js:146` stays on `bufferSubData` | Holds **only** when the shorter live view is actually installed as `data`. Before the 2026-09-11 fix, `_updateSlot` passed the full capacity array with a smaller `size`, so `data.length` never shrank and a "longer label → shorter label" update drew the stale tail (`STDILLATOR` → `STD` rendered `STDILLATOR`). Regression test: `tests/v8/slug/incremental-rebuild.spec.ts` "draw count" block. |
