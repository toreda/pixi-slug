 ...
 */
function(curveTexture,bandTexture){const glProgram=pixi_js_1.GlProgram.from({vertex:vert_glsl_1.default,fragment:frag_glsl_1.default});return new pixi_js_1.Shader({glProgram,resources:{uCurveTexture:curveTexture.source,uBandTexture:bandTexture.source}})}
 ...
 */
class SlugText extends pixi_js_1.Container{constructor(text,font,fontSize=defaults_1.Defaults.FONT_SIZE){super(),this._text=text,this._font=font,this._fontSize=fontSize,this._color=[1,1,1,1],this._mesh=null,this._curveTexture=null,this._bandTexture=null,this.rebuild()}
...
const bandRows=Math.ceil(this._font.bandData.length/4/textureWidth)||1;this._bandTexture=new pixi_js_1.Texture({source:new pixi_js_1.BufferImageSource({resource:(()=>{const f=new Float32Array(this._font.bandData.length);for(let i=0;i<this._font.bandData.length;i++)f[i]=this._font.bandData[i];return f})(),width:textureWidth,height:bandRows,format:"rgba32float",autoGenerateMipmaps:!1,scaleMode:"nearest",alphaMode:"no-premultiply-alpha"})});const shader=(0,shader_1.slugShader)(this._curveTexture,this._bandTexture),mesh=new pixi_js_1.Mesh({geometry,shader});this._mesh=mesh,this.addChild(mesh)}destroy(){this._mesh?.destroy(),this._curveTexture?.destroy(),this._bandTexture?.destroy(),super.destroy()}}exports.SlugText=SlugText},
