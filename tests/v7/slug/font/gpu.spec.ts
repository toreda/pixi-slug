/**
 * Integration tests for `slugFontGpuV7`'s parallel-compile decision
 * logic, mirroring `tests/v8/slug/font/gpu.spec.ts`. `@pixi/core`,
...
import {slugFontGpuV7} from '../../../../src/v7/slug/font/gpu';
...
describe('slugFontGpuV7 parallel-compile path', () => {
	beforeEach(() => {
	...
	describe('renderer omitted (legacy path)', () => {
		it('does not invoke slugCompileAndInject', () => {
			const cache = slugFontGpuV7(makeFont());
		...
		it('does not query getExtension when no renderer is supplied', () => {
			expect(() => slugFontGpuV7(makeFont(), null, null)).not.toThrow();
			expect(mockCompileAndInject).not.toHaveBeenCalled();
		});
	});
	...
	describe('renderer supplied + toggle off', () => {
		it('skips the parallel path entirely; no extension probe', () => {
			...
			const cache = slugFontGpuV7(makeFont(), null, renderer);
