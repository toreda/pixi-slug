 ...
 *
 * `renderer` is typed as `unknown` because the v6 `Renderer` shape is
 * not in scope here (we don't import `@pixi/core` from this module).
 * It's handed straight to {@link SlugFonts.attachRenderer}, which
 * takes `unknown` and lets the prewarm hook do the structural check.
 */
...
export class SlugApplicationPluginV6 {
	public static init(this: ApplicationV6Like, _options?: unknown): void {
		...
		// Register the app's renderer so the v6 prewarm hook can compile
