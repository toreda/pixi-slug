 ...
 *
 * PixiJS v6 surface used here: `BaseTexture.fromBuffer`, `Texture`, and
 * `WRAP_MODES`.
 */
export interface SlugFillGpuV6 {
...
/**
 * Translate a resolved fill into the GPU resources needed by the v6
 ...
 *
 * The caller (v6 SlugText.rebuild) owns the returned record and MUST
 * call `dispose()` on the previous record before installing a new one.
 */
export function slugBuildFillGpuV6(fill: SlugFillResolved): SlugFillGpuV6 {
	if (fill.kind === 'solid') {
		return {
			gradient: null,
			texture: null,
