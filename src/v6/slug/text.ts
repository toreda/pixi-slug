 ...
 * §5.5 (v8 refactor — v6 mirrors the design).
 */
...
export class SlugText extends SlugTextV6Base {
	...
	/**
	 * Most recent `SlugFontGpuV6` cache record this SlugText has
	...
	constructor(init: SlugTextInit) {
		...
		// hides PIXI v6 DisplayObject's interactivity fields.
	...
	/**
	 * Hit-testing note: the geometry built below uses a Slug-specific
	 * attribute layout (`aPositionNormal`, `aTexcoord`, `aJacobian`,
	 * `aBanding`, `aColor`) rather than PIXI's stock `aVertexPosition`.
	 * PIXI's `Mesh.containsPoint` looks up `aVertexPosition` directly
	 * and crashes on this geometry, which is why the parent SlugText
	 * disables hit-testing in its constructor (`interactive = false`,
	 * `interactiveChildren = false`). Anything that flips those back
	 * on needs a custom `containsPoint` or a separate hit-test
	 * rectangle — do not assume PIXI's default works on these meshes.
	 */
	private _buildMesh(
	 ...
	 * defers the binding that triggers PIXI v6's blocking
	 ...
	 * churn an array per rebuild. v6 calls `mesh.destroy()` to release
	 * per-mesh GPU buffers — v6's `Program.glPrograms` cache is keyed on
	 * the program (not on the mesh), so destroying meshes does not
	 * affect the shared compiled program.
	 */
	...
	protected _render(renderer: Renderer): void {
		...
		if (gpu.programReady && this._programReadyCache !== gpu) {
			...
			gpu.programReady.then(() => {
				...
				// take the attach path. v6 has no `onRender` setter to
				// clear/re-arm — `_render` fires every frame regardless.
			});
			return;
		}
