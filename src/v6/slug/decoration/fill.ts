...
/**
 * Texture-fill descriptor for a decoration rect, in PixiJS v6's
 * `Graphics.beginTextureFill` form. Returned by
 * `slugBuildDecorationFillV6` when the resolved fill is a texture; null
 * otherwise (the caller falls back to a solid color).
 *
 * GRADIENTS ARE NOT SUPPORTED on v6 decorations. v6 `Graphics` has no
 * native gradient fill (v8 added `FillGradient`). Rendering a gradient
 * would require baking the LUT into a `RenderTexture`, which needs a
 * Renderer reference at rebuild time that `text.ts` does not currently
 * hold. Decorations on a gradient-filled SlugText fall back to the
 * representative solid color from `_color`.
 */
export interface SlugDecorationTextureFillV6 {
	texture: Texture;
	matrix: Matrix;
	alpha: number;
}
...
/**
 * Build a v6 texture-fill descriptor sized to the text bbox so the
 ...
 */
export function slugBuildDecorationFillV6(
...
): SlugDecorationTextureFillV6 | null {
	if (fill.kind !== 'texture') {
		// Solid → caller already draws solid. Gradients are unsupported
		// on v6 decorations; caller falls back to solid color.
		return null;
	}
